...
{ config: TrackConfig ->
    ...
    fun Sequence<Step>.tb303() = this.synth("tb303")
        .params("wave" to 0)
        .params("pan" to LFO(-0.75, 0.75, 6))

    fun Sequence<Step>.plaits() = this.synth("plaits")
//        .params("harm" to 0.5)
//        .params("morph" to 0.7)
...
//    config.tracks[0].djf(LFO(0.65, 0.89, 4))
    ...
    config.tracks[1].djf(0.0)
    config.tracks[1].comp(0.5)

    config.tracks[2].delay(level = 0.5, echo = 0.75, echotime = 6.0)
    config.tracks[2].reverb(level = 0.1, drywet = 0.3, damp = 0.2)
    config.tracks[2].djf(LFO(0.75, 0.89, 4))
//    config.tracks[2].djf(0.5)
    config.tracks[2].comp(0.7)

    config.tracks[3].delay(level = 0.35, echo = 1.25, echotime = 4.0)
    config.tracks[3].reverb(level = 0.9, drywet = 0.2, damp = 0.2)
...
//            .mute()
        ...
        + repeat(ll).loop("dl-attic", ll, startBeat = 2)
            .chop(config, 0.5)
            .mute()

        + cycle(ll).loop("solenoid_3", ll, at = 0.5)
            .waveDist(0.25)
//            .mute()
...
//            .mute()
        ...
        + listOf((0.25 to 3), (0.5 to 3), (0.25 to 4))
...
//            .mute()
        ...
        + scale.low()
            .phrase(
                degrees(
                    cycle(progression).flatMap { arp(it, 3, ArpType.UPDOWN) }
                ),
                cycle(0.25)
            )
            .every(3, Step::low, 1)
